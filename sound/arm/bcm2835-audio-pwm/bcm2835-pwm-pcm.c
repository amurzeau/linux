...
#define PWM_RATE 250000002
#define SAMPLING_RATE 400000
#define RANGE 625
...
void write_data(bcm2835_alsa_stream_t *alsa_stream, int period_size, int buffer_size) {
	int pos = alsa_stream->pos;
	int copy_left = period_size;
	int copy_size;
	
	while(copy_left > 0) {
		const int remaining = (buffer_size - pos) / 3 * 3;
		if(remaining < period_size)
			copy_size = remaining;
		else
			copy_size = period_size;
		
		
		bcm2835_pwm_aud_write(&alsa_stream->chip->chip, alsa_stream->substream->runtime->dma_area + frames_to_bytes(alsa_stream->substream->runtime, pos), copy_size);
				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	if(runtime->dma_bytes != params_period_size(params) * params_periods(params) * 4) {
		audio_error
		    (chip, "bad dma area size: %d, expected %d\n", runtime->dma_bytes, params_period_size(params) * params_periods(params) * 4);
	}
...
static int snd_bcm2835_pwm_pcm_get_time_info(struct snd_pcm_substream *substream,
